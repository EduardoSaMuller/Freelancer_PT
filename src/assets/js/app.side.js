...
(function (factory) {
	...
	MagnificPopup.prototype = {
		...
		open: function (data) {
			...
			if (data.isObj === false) {
				// convert $ collection to array to avoid conflicts later
...
(function (fn) {
	...
	var assign = Object.assign || (window.$ && $.extend);
	...
	// $ special events
	//
	// $ event objects are copies of DOM event objects. They need
...
$(".main-nav nav li.tabs-title").click(function () {
	...
	$(".main-nav").removeClass("is-closed");

	var dropdown = $(".main-nav").find(".subnav");
...
$(window).on("load", function () {
	...
	/*
  setTimeout(function() {
    $('body').removeClass('animate is-animating');
  }, 1700);
*/
	setTimeout(function () {
	...
	/*
  setTimeout(function() {
    $('.container-animation').removeClass('is-animating');
  }, 1700);
*/
	$(".before-after-container").twentytwenty({
		...
		after_label: "Depois", // Set a custom after label
...
$(document).on("click", ".accordion-title", function (ev) {
...
console.log($(this).children('.slider-handle.start').attr('aria-valuenow'));
console.log('valor final:');
console.log($(this).children('.slider-handle.end').attr('aria-valuenow'));
});
*/
...
$(document).ready(function () {
	$(".billing input").on("keyup", function () {
		let empty = false;
		...
		if (empty && emptySelect) $("#continue-to-send").attr("disabled", "disabled");
		else $("#continue-to-send").attr("disabled", false);
	});
});
...
$("button#continue-to-send").click(() => {
	$("p.summary-info").text(
		...
		${$("#id-last-name-billing").val()} , ${$("#id-morada-billing").val()} ${$(
			"#id-address-billing"
		).val()} , ${$("#id-city-billing").val()} - ${$("#id-postalcode-billing").val()}`
	);
...
$("#tos").on('change', (event) => {
	...
	$('.enviar-encomenda').on('click', submit);
	function submit() {
		// console.log("teste");
	}
})
$(document).ready(function(){
	...
	if  ($(".icon-logo-gracehome").hasClass("icon-logo-gracehome")) {
	...
	if  ($(".icon-logo-sonno ").hasClass("icon-logo-sonno")) {
